    ...
    public FlowMessage<A> process(FlowMessage<A> in) throws FlowException {
        try (Context context = Context.newBuilder("python")
            ...
            if(result.isNull()) {
                ...
                context.getBindings("python").putMember("_py_result_", result);
                Value jsonStringValue = context.eval("python", "import json; json.dumps(_py_result_)");
                in.setPayload((A)jsonStringValue.asString());
            } else {
